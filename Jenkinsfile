  ...
  hostPathVolume(mountPath: '/var/run/docker.sock', hostPath: '/var/run/docker.sock'),
   ...
   node('buildHmdaHelp') {
    ...
    stage('Deploy') {
      ...
      container('helm') {
        sh "helm upgrade --install --force --namespace=default --values=kubernetes/hmda-homepage/values.yaml --set image.tag=${gitBranch} hmda-homepage kubernetes/hmda-homepage"
      }
    }
